...
//        sws_ctx = sws_getContext(ffmpegDecoder->codecCtx->width, ffmpegDecoder->codecCtx->height, ffmpegDecoder->codecCtx->pix_fmt,
//                ffmpegDecoder->codecCtx->width, ffmpegDecoder->codecCtx->height, PIX_FMT_BGR24,
            ...
            if (len > 0)
            {
                ...
                if (frameFinished)
                {
                    if (error == ARCODECS_OK)
                    {
                        ...
                        //outputFrame->format = ARCODECS_FORMAT_RGBA;
...
//                        if (bgrFrame != NULL)
//                        {
//                            int i = sws_scale(sws_ctx,(uint8_t const * const *) ffmpegDecoder->decodedFrame->data, ffmpegDecoder->decodedFrame->linesize,
//                                    0, ffmpegDecoder->codecCtx->height,bgrFrame->data, bgrFrame->linesize);
//                            if(bgrFrame->data[0])
//                                fprintf(stderr, "%c ", bgrFrame->data[0][5]);
//
//                        }
                    }
                    else
