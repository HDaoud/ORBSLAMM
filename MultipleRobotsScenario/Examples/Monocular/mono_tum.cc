...
int main(int argc, char **argv)
{
    if(argc != 5)
    {
        cerr << endl << "Usage: ./mono_tum path_to_vocabulary path_to_settings path_to_sequence [0|1]for_Multi_Maps_Usage" << endl;
        return 1;
    }
...
//
//    iORB_SLAM::ORBVocabulary* pVocabulary = new iORB_SLAM::ORBVocabulary();
//    bool bVocLoad = pVocabulary->loadFromTextFile(argv[1]);
//    if(!bVocLoad)
//    {
...
//    iORB_SLAM::System SLAM2(pVocabulary,argv[2],iORB_SLAM::System::MONOCULAR,true, bUseMMaps);
    iORB_SLAM::System SLAM2(argv[1],argv[2],iORB_SLAM::System::MONOCULAR,true,bUseMMaps);
...
{
    ifstream f;
    string location = strFile +"/rgb.txt";
...
//    SLAM.SaveMultipleMapsTrajectories("Maps.txt");//"MMapsTrajectories_"+date+".txt");
}
...
void RunSLAM2(int& start, int& nImages, iORB_SLAM::System& SLAM, vector<string>& vstrImageFilenames, vector<double>& vTimestamps)
{
    ...
    date = time_s;
    // Save camera trajectory
    SLAM.SaveKeyFrameTrajectoryTUM("KeyFrameTrajectory_"+date+"2.txt");
//    SLAM.SaveMultipleMapsTrajectories("Maps.txt");//"MMapsTrajectories_"+date+".txt");
}